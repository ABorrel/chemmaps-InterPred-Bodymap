...
from os import path, remove, listdir, system, chdir, getcwd, makedirs
...
PADELXML = "/home/aborrel/django_server/django_server/interferences/descriptor/desc_fp.xml"
...
def runPadel(prin="", typerun="2D"):
    """Input include a folder of sdf file"""
    if typerun == "2D":
        pfilout = prin + "tem2D.csv"
    elif typerun == "FP":
    ...
    if prin == "":
        ...
        if typerun == "2D":
            ...
            cmd = "java -jar " + PADEL + " -fingerprints -descriptortypes " + str(PADELXML) + " -removesalt -standardizenitro -detectaromaticity -retainorder -maxruntime 10000 -dir " + str(prin) + " -file " + pfilout
...
def computeOperaDesc(psdf, prSession):
    ...
    # run PADEL
    pdescPADEL = runPadel(prOPERA, "2D")
    pFPPADEL = runPadel(prOPERA, "FP")
    ...
    for pdesc in lpdesc:
        try:
            ddesc = loadMatrixToDict(pdesc, ",")
        except:
            print("ERROR pdesc -> " + pdesc)
            return 1

        for chem in ddesc.keys():
            if not chem in dopera.keys():
                dopera[chem] = {}
            for desc in ddesc[chem].keys():
                if desc in LOPERA:
                    dopera[chem][desc] = ddesc[chem][desc]


    for chemPADEL in ddescPADEL.keys():
        for descPADEL in ddescPADEL[chemPADEL].keys():
            if descPADEL in LOPERA:
                dopera[chemPADEL][descPADEL] = ddescPADEL[chemPADEL][descPADEL]


    pfilout = prSession + "OPERA.csv"
    filout = open(pfilout, "w")
    filout.write("ID\t" + "\t".join(LOPERA) + "\n")
    for chem in dopera.keys():
        filout.write("%s\t%s\n"%(chem, "\t".join(dopera[chem][desc] for desc in LOPERA)))
    filout.close()
...
def runOPERA(psdf, p2Ddesc, pFP, prtemp):
    ...
    if len (listdir(prtemp)) == 3:
        ...
        #cmd = "%s %s -s %s -o %s -StrP -BCF -BP -logP -MP -VP -logVP -WS -AOH -BioDeg -RB -ReadyBiodeg -HL " \
        #      "-logHL -KM -logKM -KOA -Koc -logKoc -RT -logD" % (OPERA, MATLAB, psdf, ppred)
...
def getLdesc (typeDesc):
    ...
    if typeDesc == "1D2D":
        # listdesc
        lout = lCONSTITUTION + LCOMPO + LMOLPROP + LTOPOLOGY + LCONNECTIVITY + LKAPA + LBUCUT + LBASAK + LESTATE + \
               LMOREAUBROTO + LMORAN + LGEARY + LCHARGE + LMOE
...
class Descriptor:
    ...
    def computeAll2D(self):
        ...
        # remove control => have to be more precise
        #if path.exists(self.pdesc + "_2D.txt"):
        #    if path.getsize(self.pdesc + "_2D.txt") > 100:
        #        self.all2D = loadMatrixToDict(self.pdesc + "_2D.txt")
        #        return
        #    else:
