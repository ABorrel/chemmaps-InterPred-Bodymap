...
class CASUpload(forms.Form):
    ...
    lname = cDB.execCMD("SELECT DISTINCT casn, name from %s.bodymap_chemsum WHERE name is not NULL ORDER BY name"%(cDB.schema))
    lcas = cDB.execCMD("SELECT DISTINCT casn, casn from %s.bodymap_chemsum WHERE casn is not NULL ORDER BY casn"%(cDB.schema))
