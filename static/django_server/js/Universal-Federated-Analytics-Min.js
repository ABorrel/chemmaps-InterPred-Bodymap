...
function _defineCookieDomain() {
    ...
    if (domainPattern.test(oCONFIG.SUBDOMAIN_BASED.toString())) {
        ...
        if (oCONFIG.SUBDOMAIN_BASED.toString() == 'false') {
            ...
            oCONFIG.SUBDOMAIN_BASED.toString() == 'auto' ||
            oCONFIG.SUBDOMAIN_BASED.toString() == 'true'
        ) {
...
function _updateConfig() {
    ...
    for (var i = 0; i < _paramList.length; i++) {
        ...
        switch (_key) {
            ...
            case 'dclink':
                _value = _cleanBooleanParam(_value);
                if (true == _value || false == _value) oCONFIG.DOUNBLECLICK_LINK = _value;
                break;
            case 'enhlink':
                _value = _cleanBooleanParam(_value);
            ...
            try {
                window[window['GoogleAnalyticsObject']](
                    ...
                    'pageview',
                    {
                        page: _virtualPath,
                        title:
                            _virtualTitle != '' || _virtualTitle != undefined
                                ? _virtualTitle
                                : document.title,
                    }
                );
    ...
    if (
        ...
        if (_hitType.toLowerCase() == 'pageview') {
            try {
                _sendPageview(
                    _param1,
                    _param2 == undefined || _param2 == '' ? document.title : _param2
                );
            } catch (err) {}
            ...
            try {
                ...
                if (mtrcsArr.length > 0) {
                    _sendCustomMetrics(
                        mtrcsArr,
                        _param1 == undefined || _param1 == '' || isNaN(_param1)
                            ? 1
                            : parseFloat(_param1)
                    );
                }
    ...
    var onYouTubePlayerAPIReady = function() {
        for (var i = 0; i < videoArray_fed.length; i++) {
            playerArray_fed[i] = new YT.Player(videoArray_fed[i], {
                events: { onReady: onFedPlayerReady, onStateChange: onFedPlayerStateChange },
            });
        }
    };
