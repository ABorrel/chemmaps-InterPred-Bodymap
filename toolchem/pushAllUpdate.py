...
class pushAllUpdate:
    ...
    def pushChemicalDescription(self):
        ...
        if l_map_full_coords != []:
            for map in l_map_full_coords:
                ...
                cmd_update_coords = "UPDATE chemical_description SET dim1d2d = chemical_description_user.dim1d2d, dim3d = chemical_description_user.dim3d, d3_cube=chemical_description_user.d3_cube FROM chemical_description_user WHERE chemical_description.inchikey=chemical_description_user.inchikey AND chemical_description.map_name='%s' AND chemical_description_user.map_name='%s' AND chemical_description_user.status='coords'"%(map_in, map_in)
        ...
        for inchikey in d_chem_user.keys():
            ...
            cmd_sql = "UPDATE chemical_description SET %s FROM chemical_description_user WHERE chemical_description.inchikey='%s' AND chemical_description_user.inchikey='%s' AND chemical_description.map_name='%s' AND chemical_description_user.map_name='%s' AND chemical_description_user.status != 'coords'"%(",".join(l_cmd_update), inchikey, inchikey, d_chem_user[inchikey]["map_name"] ,d_chem_user[inchikey]["map_name"])
            self.cDB.DB.updateElement(cmd_sql)
        self.cDB.closeConnection()
    ...
    def recomputeNeighbors(self, nb_neighbors=20):
       ...
       # extract chemical that need neighbor computation
        ...
        while i < imax:
            cmdExtract = "Select inchikey from chemical_description WHERE map_name = '%s' ORDER BY cube(d3_cube) <->  (select cube (d3_cube) from chemical_description where id='%s' AND map_name = '%s' limit (1))  limit (%s);"%(d_chem[l_id[i]]["map"], l_id[i], d_chem[l_id[i]]["map"], nb_neighbors + 1)
